...
def main():
    ...
    parser.add_argument('--range', type=str, help='Range 1 or 2 for daily mode - 1 is 0 to 12 hours, 2 is 12 to 24 hours data')
    ...
    parser.add_argument('--retain-file', type=lambda x: x.lower() in ('true','1','yes'), default=False,
    ...
    parser.add_argument('--log-run', type=lambda x: x.lower() in ('true','1','yes'), default=False,
    ...
    load_dotenv()
    log_config_file = 'config/logging_debug.yaml' if args.debug else 'config/logging.yaml'
    setup_logging(log_config_file)

    if (args.startdate == args.enddate) and args.startdate:
        logger.info(f"Start date {args.startdate} is the same as end date {args.enddate}. Triggering daily mode.")
    ...
    if args.mode == 'live':
        ...
        if start_date > end_date:
            ...
            sys.exit(1)
        dates_list = [start_date + datetime.timedelta(days=i) for i in range((end_date - start_date).days + 1)]
        log_run_to_localdb = args.log_run # True if args.log_run else False. Also False is date is today.
        for dt in dates_list:
            logger.debug(f"Processing date {dt}")
            if dt == datetime.datetime.now(datetime.timezone.utc).date():
                log_run_to_localdb = False
                logger.info(f"Skip logging the run details for {dt} as yet data needs to be loaded for today's date.")
            for i in range(1,3):
