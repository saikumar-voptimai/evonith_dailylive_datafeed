...
class TestMain(unittest.TestCase):
    ...
    def test_main_live_mode(
        ...
        with patch(
            "src.main.CONFIG",
            {
                "DATE_FORMAT_FILENAME": "%m_%d_%Y",
                "TIME_FORMAT_FILENAME": "%H_%M_%S",
                "WAIT": 10,
            },
        ):
            src.main.main()
    ...
    @patch("src.main.load_dotenv")
    @patch("src.main.argparse.ArgumentParser.parse_args")
    def test_main_daily_mode(
        self,
        mock_args,
        mock_dotenv,
        mock_log_yaml,
        mock_process_datewise,
        mock_setup_logging,
        mock_init_db,
    ):
    ...
    def test_main_missing_date(
        ...
        expected_yesterday = (
            (datetime.now(timezone.utc) - timedelta(days=1)).date().strftime("%m_%d_%Y")
        )
        expected_yesterday_dt = datetime.strptime(expected_yesterday, "%m_%d_%Y")
        assert (
            called_args == expected_yesterday_dt
        ), f"Expected {expected_yesterday_dt}, got {called_args}"
